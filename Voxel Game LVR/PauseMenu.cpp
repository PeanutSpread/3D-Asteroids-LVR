	...
	, _scale(scale)
{
	...
	_materials.push_back(new Material(_textures[5]));
...
void PauseMenu::_draw() {
	...
	delete _cursor;
	_cursor = new Entity(_meshes[3], _materials[5], Transform(_mouse->getPosition())); // Cursor
	_cursor->setOrientation(_orientation);

}

void PauseMenu::_mouseLocator(const Mouse * mouse) {
